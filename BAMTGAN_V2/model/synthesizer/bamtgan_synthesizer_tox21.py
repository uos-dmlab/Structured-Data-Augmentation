...
from model.synthesizer.transformer_tox21 import ImageTransformer,DataTransformer
...
def determine_layers_disc(side, num_channels):
    ...
    """
    if side is None:
        # sideê°€ Noneì¸ ê²½ìš°ë¥¼ ëŒ€ë¹„í•œ ì˜ˆì™¸ ì²˜ë¦¬
        side = 1  # ì´ ê°’ì€ ìƒí™©ì— ë”°ë¼ ì ì ˆížˆ ì¡°ì •í•´ì•¼ í•©ë‹ˆë‹¤.
    ...
    def fit(self, train_data=pd.DataFrame, categorical=[], mixed={}, type={}):
        ...
        for i in tqdm(range(self.epochs)):
            for _ in range(steps_per_epoch):
                ...
                # computing the loss to essentially maximize the log likelihood of correctly classifiying real and fake samples as log(D(x))+log(1âˆ’D(G(z)))
                # or equivalently minimizing the negative of log(D(x))+log(1âˆ’D(G(z))) as done below
    ...
    def sample(self, n):
        ...
        for _ in range(steps):
            ...
            for classifier in classifiers:
                ...
                print('{0} ì •í™•ë„: {1:.4f}'.format(class_name, accuracy_score(y_test, pred)))

            voting_model.fit(X_train, y_train)
            pred = voting_model.predict(X_test)
            print('ë³´íŒ… ë¶„ë¥˜ê¸°ì˜ ì •í™•ë„: {0: .4f}'.format(accuracy_score(y_test, pred)))
