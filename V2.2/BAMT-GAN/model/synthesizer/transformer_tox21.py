...
class ImageTransformer():
    ...
    def __init__(self, side):
        ...
        if side is None or side < 4:  # 4 is the kernel size in the Conv2d layer
            self.height = 4
        else:
            self.height = side
    def transform(self, data):
...
class ImageTransformer():
    ...
    def transform(self, data):
        ...
        # calculate the total elements in the data and the elements we need to keep
        total_elements = data.numel()
        elements_to_keep = num_samples * self.height * self.height

        if elements_to_keep < total_elements:
            print("Data cannot be reshaped into (-1, 1, {}, {}). Adjusting data...".format(self.height, self.height))
            # adjust the data size
            data = data[:, :elements_to_keep // num_samples]

        # if still the data size does not match with the image size, we will add padding
        if self.height * self.height > data.shape[1]:
            # tabular data records are padded with 0 to conform to square shaped images
            padding = torch.zeros((num_samples, self.height * self.height - data.shape[1])).to(data.device)
            data = torch.cat([data, padding], axis=1)
